...
As mentioned before, object capabilities are formed by a pair of sealed capabilities. The object owner first creates an unsealed capability such that the capability's type is a pointer to the entry function relative to the region defined by the base and length, i.e. \texttt{base~+~type~=~entry\_func} and \texttt{type~<=~length}. Notice that only code which already has access to memory containing the entry function can create this capability. 
