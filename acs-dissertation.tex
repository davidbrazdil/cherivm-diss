...
Writing software in Java and other safe programming languages has inherent advantages, namely a higher guaranteed level of robustness, reliability and portability. Yet large software systems are hardly ever written in just one language as the development process can create strong incentives to save time by using third-party libraries or legacy code, or to implement some components in a more suitable language, most often in attempt to improve performance. However, the lack of safety makes native code a threat as it has the potential to compromise the entire Java runtime, including its code safety properties and core security mechanisms. The decision to include native code is therefore always a trade-off between security and the benefits it can provide.
...
To the enterprise environment, where security tends to be prioritized, Java is a platform capable of executing untrusted applications inside a safe environment that limits its access to system resources (a \emph{sandbox}), and even running multiple such applications side-by-side inside one instance of the virtual machine. But the inability to constrain native code in the same manner dictates that only applications written eclusively for the Java Virtual Machine (JVM) can be controlled this way.
