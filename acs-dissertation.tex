...
The process of loading a native library is initiated by calling the \texttt{System.loadLibrary} method. The VM looks for a shared object file of the given name inside lookup folders and attempts to load it with the mechanism approprite on that platform, e.g.\ \emph{libdl} on variants of UNIX. When running on CHERI, this mechanism is now defined to be \emph{libcheri} -- a library the CHERI SDK provides for managing sandbox instances. \emph{libcheri} begins by reserving a memory region for the newly created sandbox, prepares the address space layout as summarized in Table~\ref{table:addressSpaceLayout} and memory maps the \emph{sandbox image} (library file) into that region. If successful, it returns an object capability which defines the security domain. The ABI convention for CHERI dictates that the entry point is at \texttt{0x8000}.
