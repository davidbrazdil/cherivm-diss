...
In order to retain source-level backwards compatibility, the native code can be compiled against \emph{libqishr} which provides wrappers for \emph{libc} functions and the \texttt{JNIEnv} interface, and also automatically manages the capabilities passed to the sandbox, allowing for seamless integration of legacy code into the sandboxed environment.
...
For the time being, Qishr uses a simple workaround. When the entry function is invoked with \texttt{1} in \reg{a0}, the sandbox should extract a function name string from a read-only capability in \reg{C1} and look it up in its own symbol table. If found, it should return a non-zero handle, otherwise it should return \texttt{NULL}. The implementation in \emph{libqishr} will look for the symbol in a list of function name and pointer pairs pregenerated by the \emph{Ctags}\footnote{\url{http://ctags.sourceforge.net/}} code analyzer and a provided script. The handle is therefore the pointer to the function relative to the base of the inner address space.
