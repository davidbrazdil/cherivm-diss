...
Robusta (\S\ref{sec:Robusta}) reports 2-10\% overhead for similar operations, but the authors evaluated it against OpenJDK which does not support pinning. Tests were therefore re-run on JamVM with pinning disabled and a comparable overhead of was 4-8\% measured. In both cases, this is the slowdown of downcalls, upcalls, access control and with Robusta also the dynamic checks of SFI. Because CHERI does not have a similar always-present overhead, Qishr should -- and sometimes does -- perform slightly better. The slow JamVM interpreter involved in access control is likely to be the reason why this difference is not more significant.
