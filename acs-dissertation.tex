...
\begin{table}
	...
	\begin{tabular}{|l|c|c|c|}
		...
		file server (w/o pinning)	& $(0.0 \pm 0.0)$ s	& $(118.8 \pm 1.6)$ s 	& $(0.0 \pm 0.0)$ \% \\
...
Robusta (\S\ref{sec:Robusta}) reports 2-10\% overhead for a similar type of operations\footnote{GNU Classpath implements \class{ZipFile} class in Java, not as a \emph{zlib} wrapper. Same compression tests were not reproducible.}, but the authors evaluated it against OpenJDK which does not support pinning. Tests where therefore re-run on JamVM with pinning disabled and a comparable overhead of 6-10\% measured. 
