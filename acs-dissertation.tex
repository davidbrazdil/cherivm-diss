...
Because sealed capabilities cannot be tampered with, only passed around, these truly opaque and unforgeable data structures are ideal for representing native handles. The unforgeability is given by the fact that the the \insn{CSealData} instruction copies the object type field from the sealing capability to the result. This field is a 64-bit pointer within the bounds of the sealing capability. Therefore, by setting this field to a location that the sandbox does not have acces to, the sandbox will not be able to create the sealing capability and consequently forge the handles. Having multiple sealing capabilities also enables the sandbox to easily distinguish between different types of native handles.
...
Safe handles protect against arbitrary access to the address space, but they do not enforce the rules of access control among Java objects, which still allows native code to request handles of class members which should not be visible to it. The reason why this was not a part of the JNI specification is that native code could easily modify the Java stack and hence change the \emph{execution context}, i.e.\ the class it appeared to be a member of. Since the Java stack is now also protected by the sandbox, the about execution context information can be trusted and Qishr can enforce this policy at the \texttt{JNIEnv} trampoline. Native code still has the option of using reflection, just like managed code, but the difference is that the use of reflection can be forbidden by the security manager.

This way, Qishr can guarantee the memore safety of native code towards Java objects and other internal data structures. Additionally, Qishr can assist as a debugging tool because any violation of these properties will be discovered either by the compiler or will cause traps at runtime.

\subsection{Memory Management}

\subsubsection{Local and Global References}
