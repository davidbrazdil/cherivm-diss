...
function varargout = hbarwitherr(errors,absY,varargin)
...
if nRows > 1
    ...
    for col = 1:nCols
        ...
        hErrorbar(col) = errorbar(mean(x,1), absY(xOrder,col), lowerErrors(xOrder,col), upperErrors(xOrder, col), '.k');
        set(hErrorbar(col), 'marker', 'none')
    end
else
    x = get(get(handles.bar,'children'),'xdata');
    hErrorbar = errorbar(mean(x,1), absY, lowerErrors, upperErrors, '.k');
